...
L.ExportControl = L.Control.extend({
	...
	onAdd: function( map ) {
		...
		if( this.options.types && this.options.titles ) {
			...
			this._ajax(this.options.endpoint + this.options.getFormatFrom, function(res) {
	...
	_updateVariants: function() {
		...
		for( i = 0; i < types.length; i++ ) {
			...
			var stop = L.DomEvent.stopPropagation;
			if( codeid ) {
				link1.href = this.options.endpoint + codeid + '&format=' + types[i] + '&mapid=' + this.options.mapid;
				L.DomEvent
					.on(link1, 'click', stop)
	...
	_linkClick: function(e) {
		if(this.options.mapid == 0) {
			e.preventDefault();
			Alert("Please save the map before downloading", "info");
		}
